				...
				actions : {
							...
							success : function (aEntityData) {
								this.waitFor({
									...
									success : function (oList) {
										var sCurrentId,
											aItemsNotInTheList = aEntityData.filter(function (oObject) {
												return !oList.getItems().some(function (oListItem) {
													return oListItem.getBindingContext().getProperty("IssueId") === oObject.IssueId;
												});
											});

										if (!aItemsNotInTheList.length) {
											// Not enough items all of them are displayed so we take the last one
											sCurrentId = aEntityData[aEntityData.length - 1].IssueId;
										} else {
											sCurrentId = aItemsNotInTheList[0].IssueId;
										}
										...
										oCurrentItem.bindingPath = "/" + oList.getModel().createKey("Issues", {
											IssueId : sCurrentId
										});
					...
					iRememberTheListItem : function (oListItem) {
						...
						this.getContext().currentItem = {
							...
							id: oBindingContext.getProperty("IssueId"),
							title: oBindingContext.getProperty("IssueDescription")
						};
					}
				},
				...
				assertions : {
					...
					theListHeaderDisplaysZeroHits : function () {
						return this.waitFor({
							viewName : sViewName,
							id : "page",
							matchers : new PropertyStrictEquals({name : "title", value : "Issues (0)"}),
							success : function () {
								Opa5.assert.ok(true, "The list header displays 'Issues (0)'");
							},
					...
					theListShouldBeSortedAscendingOnName : function () {
						return this.theListShouldBeSortedAscendingOnField("IssueDescription");
					},
					...
					theListShouldHaveAllEntries : function () {
						...
						// retrieve all Issues to be able to check for the total amount
						...
						return this.waitFor({
							...
							success : function (oList) {
								...
								this.waitFor({
									id : "page",
									viewName : sViewName,
									matchers : new PropertyStrictEquals({name : "title", value : "Issues (" + iExpectedLength + ")"}),
									success : function () {
